...
| `espn-player-stats.json` | `player-stats.ts` | Receiving lines for three seasons (**real**: Jeremiah Smith, captured 2026-09-24): one entry per season, oldest first, the last one the season in progress, as the endpoint sends them from September. NFL athletes come back in the same shape (checked against the Chiefs, same day). Asked for 2025, the answer is the middle line, so a parser taking the first or the last fails. An all-zero category, and a category whose only line is 2023, so a test asking for 2025 drops it and one asking for 2023 keeps it |
